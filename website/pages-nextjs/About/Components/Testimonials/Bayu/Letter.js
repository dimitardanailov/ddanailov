function BayuLetter() {
  return (
    <>
      <p>
        WIth his experience, Mitco able to show awesome skill, knowledge and
        critical thinking during working with him. He could play solo and also
        collaboratively which making him easier to adapt various condition and
        challenge.{' '}
      </p>
      <p>
        He also loves to give some advice during discussion and in the same time
        open for any input which making us grow together as a team. Will all of
        this experience,
      </p>
      <p>
        I believe he would be a great asset to any company or team. Good luck
        Mitco!!!
      </p>
    </>
  )
}

export default BayuLetter
